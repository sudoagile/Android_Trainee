...
SubAlgoritmo PantallaInicial
	banner;
	matrix;
	centrar("Evaluación Intermedia  Modulo 2", "<< Renato Díaz B. :: Grupo 2 >>");
	Esperar Tecla;
	Borrar Pantalla;
FinSubAlgoritmo
SubAlgoritmo matrix
...
SubAlgoritmo banner
	...
	texto = "I-logic Software Corporation, presenta:";
	
	max = Longitud(texto);
	j = 0;
	Para i=0 Hasta max-1 Hacer
		Escribir Sin Saltar SubCadena(texto,i,i);
		esperar 130 Milisegundos; 
	FinPara
...
SubProceso centrar(txt1, txt2)
	Definir h_espacios, v_espacios, i, j Como Entero;
	h_espacios = redon((58 - Longitud(txt1)) / 2);
	v_espacios = redon(8 / 2);
	Para i= 0 Hasta v_espacios Hacer
		Escribir " ";
	FinPara
	para j = 0 hasta h_espacios Hacer
		Escribir sin saltar " ";
	FinPara
	
	h_espacios = redon((58 - Longitud(txt2)) / 2);
	Escribir txt1;
	para j = 0 hasta h_espacios Hacer
		Escribir sin saltar " ";
	FinPara
